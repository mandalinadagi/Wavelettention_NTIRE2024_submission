...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        # Model "wavelettention", from tema17
        from models.team17_Wavelettention import Wavelettention
        name, data_range = f"{model_id:02}_Wavelettention", 1.0
        model_path = os.path.join('model_zoo', 'team17_Wavelettention.pth')
        model = Wavelettention()
        model.load_state_dict(torch.load(model_path), strict=True)
